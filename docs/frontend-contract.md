...
Vite + React 19 + TypeScript at repo root. Zustand persist for profile / packs. Photo blobs in IndexedDB. `src/services/api.ts` (`LabasApi`) + `src/services/http-api.ts`. Local API: `npm run server` on :8787; Vite proxies `/api`. See [api.md](api.md).
...
| Courtier (role picker → `/desk`) | Broker desk | Same dossier ids; desk-only types in `src/domain/desk.ts` |
...
| 3 | broker desk at `/desk`; same ids; desk types in `src/domain/desk.ts`; HTTP in [api.md](api.md); queue search/filter, handoff, timeline events |
