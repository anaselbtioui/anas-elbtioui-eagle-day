...
Motorist experience owns the core file. Broker desk consumes the **same dossier** (desk types in `packages/domain/src/desk.ts`).
...
`apps/web/src/services/api.ts` (`LabasApi`) is the contract. `httpApi` talks to `pnpm --filter @labas/api start`. Field names must match this doc. Photos never uploaded.
...
Desk types live in `packages/domain/src/desk.ts`. Persisted beside the motorist file (`desk_files` in Supabase / JSON). Same `Dossier.id`. No second copy. No fault / garantie / payout fields.
